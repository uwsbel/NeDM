#!/usr/bin/env python3
"""Staged global RGB-D capture and headless large-terrain Chrono collection.
...
def run_chrono(args):
            ...
            if sub == 0 and frame >= 0:
                ...
                if getattr(args, "frame_observer", None) is not None:
                    args.frame_observer.on_frame(scene, frame, state, pose, action,
                        command_context={"desired_speed_mps": 0. if at_end else float(speed[wp]), "parked": bool(at_end)})
            if frame >= 0:
                if getattr(args, "frame_observer", None) is not None:
    ...
    summary = {"case_id": case["id"], "status": status, "goal_reached": goal_time is not None,
        ...
        "measurement_scope": "Headless fixed-reference collection; static RGB-D attached in a separate verified join. No online replanning.",
...
def simulation_provenance(chrono, case_path, arena, args, dt):
    ...
    if fingerprint:
        ...
        runtime_files = [Path(chrono.__file__), Path(veh.__file__), Path(sens.__file__)]
        runtime_files += list(Path(chrono.__file__).parent.glob("*.so"))
        runtime_files += list((Path(args.chrono_data).resolve().parent / "lib").glob("libChrono*.so"))
        runtime = {str(p.resolve()): sha256(p) for p in sorted(set(runtime_files)) if p.is_file()}
...
def main():
    parser = argparse.ArgumentParser(description="Staged global RGB-D / headless Chrono collection for large terrains")
    parser.add_argument("command", choices=("observe", "collect"))
    ...
    if args.command == "collect" and not args.route:
        parser.error("--route is required for collect")
    if Path(args.out, "outcome.json").exists() or Path(args.out, "observation.npz").exists():
        parser.error("Completed output exists; use a new directory")
    args.backend, args.depth_ray_scale = "Vulkan_RT_lavapipe", 1.
    args.record_rgbd_stride, args.path_height_source = 0, "truth"
    args.frame_observer = None
    if args.command == "collect" and args.rich_telemetry:
        from nedm.traverse.fdm_rich_telemetry import RichTelemetry
        args.frame_observer = RichTelemetry(args.out, case_path=args.case, record_dt_s=DT)
    run_chrono(args)
