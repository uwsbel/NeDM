...
This version uses a fixed pre-drive global map, a 12-second default horizon,
512-pixel local sampling and 128-pixel global context, plus an attitude decoder.
...
from nedm.traverse.fdm_model import EVENT_NAMES, fdm_loss as base_fdm_loss
...
class RGBDFDMConfig:
    ...
    global_dim: int = 8
    image_size: int = 512
    global_encoding_size: int = 128
    elevation_scale_m: float = 40.0
    event_schema: str = "fdm_diverse_events_v1_asset_or_chassis"
    work_definition: str = "positive engine-interface mechanical work in kJ"
    attitude_definition: str = "signed endpoint roll/pitch in radians; not substep maxima"
    horizon: int = 60
    dt: float = 0.2
...
class RGBDFiniteHorizonFDM(nn.Module):
    ...
    Physical input tensors (horizon configurable, default 12 seconds):
      rgbd[B,4,512,512]: RGB 0..1 plus calibrated observed-depth channel;
      history[B,16,24]: causal vehicle state and preceding applied controls;
      commands[B,H,5]: nominal ego x/y, sin/cos heading, desired speed;
      global_features[B,8]: route/driver context and measured anchor pose;
      nominal_pose[B,H,4]: nominal x/y/sin/cos in the anchor vehicle frame.

    Outputs are trajectory[B,H,4], cumulative positive work[B,H,1] in kJ,
    event_logits[B,H,3], and signed roll/pitch attitude[B,H,2] in radians. Class weighting is corrected by the shared loss;
    ...
    def __init__(self, config: RGBDFDMConfig, normalization: dict[str, Any] | None = None):
        ...
        self.register_buffer("attitude_scale", torch.tensor([math.pi, math.pi / 2], dtype=torch.float32))
        ...
        spatial_dim = (config.global_encoding_size // 16) ** 2 * c * 2
        ...
        self.output_head = nn.Linear(config.decoder_hidden, config.horizon * 9)
        nn.init.normal_(self.output_head.weight, std=0.001)
        nn.init.zeros_(self.output_head.bias)
        with torch.no_grad():
            self.output_head.bias.reshape(config.horizon, 9)[:, 3] = -1.0
    ...
    def encode_image(self, rgbd: torch.Tensor, control: str = "normal") -> torch.Tensor:
        ...
        context_pixels = F.interpolate(pixels, size=(self.config.global_encoding_size, self.config.global_encoding_size),
                                       mode="bilinear", align_corners=False, antialias=True)
        return self.image_projection(self.image_encoder(context_pixels))
    ...
    def candidate_patch_pixels(self, rgbd: torch.Tensor, nominal_pose: torch.Tensor,
        ...
        if geometry_uses_depth:
            for _ in range(self.config.patch_depth_iterations):
                ...
                elevation = torch.where(valid, z.clamp(-1, 1) * self.config.elevation_scale_m, torch.zeros_like(z)).reshape_as(world_x)
        grid = project(elevation)
    ...
    def predict_from_context(self, context: torch.Tensor, commands: torch.Tensor,
        ...
        raw = self.output_head(self.decoder(hidden.flatten(1))).reshape(-1, self.config.horizon, 9)
        ...
        return {"trajectory": trajectory, "work": increments.cumsum(1), "event_logits": raw[..., 4:7],
                "attitude": torch.tanh(raw[..., 7:9]) * self.attitude_scale}
...
def fdm_loss(model, output, batch, weights):
    """Existing motion/work/event losses plus measured masked roll/pitch error."""
    total, components = base_fdm_loss(model, output, batch, weights)
    target, mask = batch["attitude"], batch["attitude_mask"]
    delta = output["attitude"] - target
    wrapped = torch.atan2(torch.sin(delta), torch.cos(delta))
    error = F.smooth_l1_loss(wrapped / .5, torch.zeros_like(wrapped), reduction="none")
    effective = mask.expand_as(error)
    if "sample_weight" in batch:
        effective = effective * batch["sample_weight"].reshape(-1, 1, 1)
    attitude_loss = (error * effective).sum() / effective.sum().clamp_min(1)
    components["attitude"] = attitude_loss
    total = total + weights.get("attitude", .5) * attitude_loss
...
def load_rgbd_checkpoint(path: str | Path, device: str | torch.device = "cpu") -> tuple[RGBDFiniteHorizonFDM, dict[str, Any]]:
    ...
    if checkpoint.get("model_family") != "diverse_rgbd_reference_gru":
