#!/usr/bin/env python3
"""One static, vehicle-free overhead RGB-D capture for ANY arena of the f104 terrain family.

Identical to scripts/capture_traverse_f104_map.py (same camera, lights, terrain colour, Vulkan lavapipe renderer,
encoding) except the arena gate: the BMP must be listed in scripts/gen_arenas.json and the patch height range is read
from the arena's own metadata instead of f104's fixed [-1.8, 3.9] m. Sensor pixels are retained unchanged; the
heightmap is used only for the audit, never in the saved observation.
"""
...
def run(a):
    ...
    allowed=json.loads((Path(__file__).resolve().parent/'gen_arenas.json').read_text())
    assert allowed.get(arena.name)==sha(arena/meta['bmp']),'arena BMP not in gen_arenas.json'
    a.bmp_sha256=sha(arena/meta['bmp']);hmin,hmax=float(meta['height_min_m']),float(meta['height_max_m'])
    ...
    dump(a.out/'observation.json',{'schema':'f104_family_static_rgbd_v1','arena':arena.name,'camera':camera,'arena_bmp_sha256':a.bmp_sha256,
      'arena_meta_sha256':sha(arena/'arena_meta.json'),'observation_sha256':sha(a.out/'observation.npz'),
      'native_geometry':{'length_m':80.,'width_m':80.,'height_min_m':hmin,'height_max_m':hmax,'sample_count':4096,'p95_abs_error_m':p95,'rmse_m':float(np.sqrt(np.mean(error**2)))},
...
if __name__=='__main__':
 p=argparse.ArgumentParser();p.add_argument('--source-root',type=Path,required=True);p.add_argument('--arena',type=Path,required=True);p.add_argument('--bmp-sha256',default='');p.add_argument('--out',type=Path,required=True);p.add_argument('--chrono-data',required=True);run(p.parse_args())
