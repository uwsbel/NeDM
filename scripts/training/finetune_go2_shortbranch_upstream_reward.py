...
# The hand-written PPO that lived here has been REMOVED. It had three algorithm bugs
# in its first hour (GAE zeroing the non-terminal flag, the critic sharing the policy
# optimiser and being throttled to 6e-6, one grad-norm clip over both), it scored
# -2.7% where this project's earlier hand-written PPO recorded -13% to -22%, and its
# weakness was quoted as evidence that analytic gradients beat sampling -- which a
# bespoke baseline of unknown quality cannot establish.
#
# Use --objective rslrl. It is the same library the base policy was trained with, the
# algorithm is the part a library gets right, and a reviewer can check it.
