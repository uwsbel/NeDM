#!/usr/bin/env bash
# Training-data-QUANTITY ablation at the OFAT Stage-A depth winner
# (L8_H8_E256_ctx128).
#
# Trains FOUR runs at the SAME L8 architecture as the mix25 generalist, changing
# ONLY the amount of training data via train_episode_fraction (a nested seeded
# subset of whole training trajectories, applied to BOTH the flat and crm train
# sources so the 75/25 mix is preserved):
#   1) L8_H8_E256_ctx128_data80  -> 80% of episodes
#   2) L8_H8_E256_ctx128_data60  -> 60%
#   3) L8_H8_E256_ctx128_data40  -> 40%
#   4) L8_H8_E256_ctx128_data20  -> 20%
# Everything else (one-hot conditioning over [flat,crm], flat normalization,
# equal-domain-combined-std Huber, rollout_sel selection, dual-domain rollout
# eval 0.5 flat + 0.5 crm, 80x2000 COMPUTE, seed 2026061801) is identical to
# L8_H8_E256_ctx128, so unique-trajectory count is the only variable. Compute is
# held fixed (sampling with replacement over the smaller pool), so this isolates
# the pure data-quantity effect; the L8_H8_E256_ctx128 run IS the 100% anchor.
...
# scripts/ablation_ofat/rank_data_quantity.py instead), so rank_stage_a.py does
...
#     'cd ~/NeDM && bash scripts/ablation_ofat/run_l8_dataquantity_ablation.sh; exec bash'
set -uo pipefail
...
DEVICE="${DEVICE:-cuda}"
SWEEP_LOG="${SWEEP_LOG:-artifacts/training_runs/ablation_ofat/l8_dataquantity.log}"
...
for entry in "${ENTRIES[@]}"; do
  ...
  for attempt in $(seq 1 "$MAX_ATTEMPTS"); do
    ...
    args=(scripts/train_hmmwv_dynamics.py --config "$config" --device "$DEVICE" --output-dir "$run_dir")
...
log "=== L8 data-quantity ablation end: completed=$completed skipped=$skipped failed=$failed ==="
[[ "$failed" -eq 0 ]]
