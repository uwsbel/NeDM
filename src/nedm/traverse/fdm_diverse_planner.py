...
class RGBDCostConfig:
    ...
    maximum_remaining_time_s: float = 360.0
    refine_families: int = 3
    max_contact_probability: float = 0.35
    max_low_progress_probability: float = 0.5
    max_rollover_probability: float = .35
    rollover_penalty_s: float = 120.
    energy_weight_s_per_kj: float = .02
    attitude_penalty_s: float = 10.
    soft_roll_deg: float = 25.
    soft_pitch_deg: float = 35.
    hard_roll_deg: float = 55.
    hard_pitch_deg: float = 55.
    ...
    def __post_init__(self):
        ...
        if not 0. <= self.max_rollover_probability <= 1.:
            raise ValueError("Rollover threshold must be in [0,1]")
        if min(self.rollover_penalty_s, self.energy_weight_s_per_kj, self.attitude_penalty_s) < 0:
            raise ValueError("Energy and attitude costs must be nonnegative")
        if not (0 < self.soft_roll_deg <= self.hard_roll_deg <= 180
                and 0 < self.soft_pitch_deg <= self.hard_pitch_deg <= 90):
            raise ValueError("Invalid soft/hard attitude limits")


class RGBDReferenceScorer:
    def __init__(self, model, rgbd, history, anchor_pose, goal_xy, *,
                 elapsed_s=0., batch_size=32, cost_config=None, control="normal"):
    ...
    def check_context(self, route):
        """Enforce the intended goal and a station consistent with localization."""
        check_reference_contract(route)
        xy = np.asarray(route["waypoints"], float)
        ss = np.asarray(route["stations"], float)
        vv = np.asarray(route["speeds"], float)
        if vv.shape != ss.shape or not np.isfinite(vv).all() or (vv < 0).any() or (vv > 6.00001).any():
            raise ValueError("Candidate speeds must follow the declared 0–6 m/s collection/planning range")
        if np.linalg.norm(xy[-1] - self.goal_xy) > .25:
            raise ValueError("Candidate endpoint differs from the supplied planning goal")
        station = route.get("meta", {}).get("fdm_station")
        if station is not None:
            station = float(station)
            if not np.isfinite(station) or station < ss[0]-1e-6 or station > ss[-1]+1e-6:
                raise ValueError("Candidate current station is outside the reference")
            point = np.asarray([np.interp(station, ss, xy[:, k]) for k in range(2)])
            nearest = float(np.linalg.norm(xy-self.anchor_pose[:2], axis=1).min())
            if np.linalg.norm(point-self.anchor_pose[:2]) > nearest+1.:
                raise ValueError("Candidate station is inconsistent with current localization")

    @torch.no_grad()
    def predict(self, routes):
        from nedm.traverse.fdm_diverse_data import build_command_features
        for route in routes:
            self.check_context(route)
        rows = [build_command_features(r, self.anchor_pose,
                    station=r.get("meta", {}).get("fdm_station"), elapsed_s=self.elapsed_s,
                    horizon=self.model.config.horizon, output_dt=self.model.config.dt)
                for r in routes]
        if not rows:
            return {}
        result = {k: [] for k in ("trajectory", "work", "event_probability", "attitude")}
        for offset in range(0, len(rows), self.batch_size):
            ...
            if getattr(self.model.config, "candidate_patches", False):
                ...
                output = self.model.predict_from_context(context, batch["commands"], batch["nominal_pose"])
            for key in ("trajectory", "work", "attitude"):
    ...
    def cost_breakdown(self, output):
        ...
        rollover = np.where(prefix, probability[..., 1], 0.).max(1) if supported[1] else np.zeros(len(distance))
        # Positive engine-interface work is learned over the finite horizon.
        # Extend its work per unit of goal progress only as an explicit cost
        # heuristic; measured full-run work determines final physical comparison.
        work = output["work"][np.arange(len(distance)), first_index, 0]
        progress = initial_distance-terminal_distance
        work_per_m = work / np.maximum(progress, 1.)
        remaining_distance = np.maximum(terminal_distance-cfg.goal_radius_m, 0.)
        estimated_work = work + np.where(reached.any(1), 0., work_per_m*remaining_distance)
        attitude = np.abs(np.degrees(output["attitude"]))
        roll = np.where(prefix, attitude[..., 0], 0.).max(1)
        pitch = np.where(prefix, attitude[..., 1], 0.).max(1)
        attitude_exposure = (np.maximum(roll/cfg.soft_roll_deg-1., 0.)**2
                             + np.maximum(pitch/cfg.soft_pitch_deg-1., 0.)**2)
        total = (estimated_time + cfg.terminal_progress_weight*(terminal_distance-initial_distance)
                 ...
                 + cfg.rollover_penalty_s*rollover + cfg.energy_weight_s_per_kj*estimated_work
                 + cfg.attitude_penalty_s*attitude_exposure)
        risk_allowed = ((contact <= cfg.max_contact_probability)
                        & (low_progress <= cfg.max_low_progress_probability)
                        & (rollover <= cfg.max_rollover_probability)
                        & (roll <= cfg.hard_roll_deg) & (pitch <= cfg.hard_pitch_deg))
        # Required collision/stall heads must be supported by actual labels.
        # With no rollover examples, only the endpoint-attitude proxy is used;
        # expose that missing event probability instead of reporting false zero.
        risk_allowed &= bool(supported[0] and supported[2])
        return {"cost": np.where(risk_allowed, total, np.inf), "unfiltered_cost": total,
                ...
                "predicted_horizon_work_kj": work, "estimated_route_work_kj": estimated_work,
                "energy_cost_s": cfg.energy_weight_s_per_kj*estimated_work,
                "predicted_peak_roll_deg": roll, "predicted_peak_pitch_deg": pitch,
                "attitude_cost_s": cfg.attitude_penalty_s*attitude_exposure,
                "predicted_goal_progress_m": progress,
                "predicted_terminal_goal_distance_m": terminal_distance,
                "contact_probability": contact if supported[0] else np.full(len(distance), np.nan),
                "low_progress_probability": low_progress if supported[2] else np.full(len(distance), np.nan),
                "rollover_probability": rollover if supported[1] else np.full(len(distance), np.nan),
                "contact_risk_supported": np.full(len(distance), supported[0], bool),
                "rollover_risk_supported": np.full(len(distance), supported[1], bool),
                "stall_risk_supported": np.full(len(distance), supported[2], bool)}
...
def propose_route_families(anchor_pose, goal_xy, *, speeds=(2., 4., 6.),
                           offsets=(0., -22., 22., -44., 44.), step_m=0.5):
...
def plan_rgbd_routes(model, rgbd, history, anchor_pose, goal_xy, *, elapsed_s=0., seed=11,
    ...
    cfg = mppi_config or MPPIConfig(samples=64, iterations=2, arena_half_extent_m=120., max_speed_mps=6.)
    ...
    return {"route": best_route, "cost": best_cost, "selected_family_index": best_family,
            ...
            "observation": "fixed pre-drive global RGB-D and causal vehicle history; no authored hazard inputs",
