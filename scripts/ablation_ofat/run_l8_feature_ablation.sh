#!/usr/bin/env bash
# Input-feature ablation at the OFAT Stage-A winner (L8_H8_E256_ctx128, S=0.0456).
#
# Trains TWO variants at the SAME L8 architecture + recipe as the mix25
# generalist, changing ONLY the input feature set:
#   1) L8_H8_E256_ctx128_no_onehot            -> drop the [flat,crm] terrain key
#                                                (input 20-D -> 18-D)
#   2) L8_H8_E256_ctx128_no_tireforce_omega   -> drop 4 tire Fz + 4 spindle omega
#                                                from state AND target, via the
#                                                body7 derived caches
#                                                (input 20-D -> 12-D, 7-D readout)
# Everything else (75/25 mix, equal-domain-combined-std Huber, rollout_sel
# selection, dual-domain rollout eval, AdamW schedule, 80x2000 steps, seed
# 2026061801, load_dataset_into_memory=false) is identical to
# L8_H8_E256_ctx128, so the feature set is the only variable.
#
# Judge these on rollout_sel, NOT val_loss: run 2 predicts 7 channels instead of
...
DEVICE="${DEVICE:-cuda}"
SWEEP_LOG="${SWEEP_LOG:-artifacts/training_runs/ablation_ofat/l8_feature.log}"
...
log "=== L8 feature ablation end: completed=$completed skipped=$skipped failed=$failed ==="
[[ "$failed" -eq 0 ]]
