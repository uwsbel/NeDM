# WP1 implementation notes — perception pilot (overnight iterations v1–v4)
...
## G1 recommendation (provisional, pending v5 on ~7.2k layouts)
...
dynamics token** (its vehicle-pose content is real and generalizes: 2.1 m /
4.4°). The v5 run (in flight, ~7.2k layouts) tests whether 3× data moves the
z₂ val BEV toward its train value; the memorization signature plus capacity
insensitivity make a rescue unlikely, but the result bounds the claim.

Provisional bars met by the spatial path on held-out layouts: occupancy IoU
0.83, rock recall 0.96, center p95 1.9 m, yaw p95 10.5°.
