...
ensembles, rigid-ground check, and the frozen recipe in eight 1 s rollout-trained
surrogates.

## Now

1. **Finish the 2048 row.** hpcfund 431212: 2048 envs seeds 1-2 and the seed-7 1 s
   surrogate at 1024 envs; score and add to STATE.

## Next

2. **Write it up** as the Study 4 results: the frozen-recipe table (eight surrogates, CRM
...
3. **Sync nodes from git.** Branch is on GitHub; have each node clone and pull instead of
   receiving `git archive` copies, so every run's code is a commit.
4. **Paths shorter than 6 s** for the three fast paths skipped identically in every arm
