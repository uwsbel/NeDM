...
from nedm.traverse.fdm_rgbd_model import (RGBD_ARMS, RGBDFDMConfig, RGBDFiniteHorizonFDM,
...
LOW_DIM_KEYS = ("history", "commands", "global_features", "nominal_pose", "trajectory", "work",
                "events", "trajectory_mask", "event_mask")
...
class Images:
    ...
    def __init__(self, path: Path, count: int, device: torch.device, image_size: int = 128,
...
def evaluate_rgbd(model: RGBDFiniteHorizonFDM, arrays: dict[str, np.ndarray], images: Images,
    ...
    if control == "shuffle" and count > 1:
        # Permute current images while preserving their RGB/depth registration.
        # A deterministic cyclic shift has no fixed points, unlike a random
        # permutation. Neither labels nor predicted risk determine the shift.
        shift = 1 + shuffle_seed % (count - 1)
        image_order = np.roll(image_order, shift)
    outputs = {key: [] for key in ("trajectory", "work", "event_logits")}
    sums, denominators = {}, {}
    ...
    if control == "shuffle":
        metrics["shuffle"] = {"seed": shuffle_seed, "fixed_points": int((image_order == np.arange(count)).sum()),
...
def save(path: Path, model: RGBDFiniteHorizonFDM, optimizer: torch.optim.Optimizer,
    ...
    checkpoint = {
        "format_version": 1, "model_family": "rgbd_reference_gru", "model_config": rgbd_model_config_dict(model),
...
def arguments() -> argparse.Namespace:
    ...
    parser.add_argument("--candidate-patches", action=argparse.BooleanOptionalAction, default=False,
    ...
    parser.add_argument("--progress-target", choices=tuple(PROGRESS_TARGET_VERSIONS), default="net_progress",
...
def run(args: argparse.Namespace) -> None:
    ...
    status = {"state": "loading", "model_family": "rgbd_reference_gru", "hostname": socket.gethostname(),
    ...
    def final_controls(model: RGBDFiniteHorizonFDM, checkpoint_step: int, filename: str,
        ...
        if "normal" in report["controls"]:
            ...
            report["image_dependence"] = {
                ...
                for control, metrics in report["controls"].items() if control != "normal"}
    ...
    cfg = RGBDFDMConfig(arm=args.arm, horizon=train["nominal_pose"].shape[1], dt=float(manifest.get("output_dt_s", 0.2)),
                       ...
                       camera_hfov_deg=float(camera.get("hfov_deg", 47.0)), camera_height_m=float(camera.get("cam_height_m", 100.0)),
    ...
    sources = [Path(__file__), ROOT / "src/nedm/traverse/fdm_rgbd_model.py", ROOT / "scripts/traverse_fdm_train.py",
               ROOT / "src/nedm/traverse/fdm_model.py", ROOT / "src/nedm/traverse/fdm_rgbd_data.py"]
    try:
    ...
    provenance = {
        ...
        "device_name": torch.cuda.get_device_name(device), "input_source": "measured current RGB-D and causal vehicle history",
    ...
    if args.resume is not None:
        ...
        if checkpoint.get("model_family") != "rgbd_reference_gru" or checkpoint["model_config"] != rgbd_model_config_dict(model):
        ...
        for key in ("seed", "batch", "positive_sampling_fraction", "max_event_pos_weight", "learning_rate", "weight_decay",
                    "xy_weight", "yaw_weight", "work_weight", "event_weight", "grad_clip", "rotation_augmentation", "progress_target"):
