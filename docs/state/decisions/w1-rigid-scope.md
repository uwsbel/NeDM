...
### The consequence that outlives this episode

**`gate_go2_action_sensitivity.py` does not set `PYTHONPATH`.** Its own two arms are
generated by the same interpreter and so remain comparable to each other -- the
gain/corr/cosine numbers are internally consistent. But its **apparatus check**
compares a freshly generated arm against the STORED recording, and those come from
different builds. That comparison mixes two binaries and its residual includes a
build difference that has nothing to do with the surrogate.

Any replay-based tool should set `PYTHONPATH` the way the driver does, or state
which build it assumes.
